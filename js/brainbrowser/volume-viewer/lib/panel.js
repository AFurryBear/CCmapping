      ...
      updateSlice: function(callback) {
        ...
        update_timeout = setTimeout(function() {
          ...
          if (volume.type=="overlay"){
            ...
            // 解决volume切换resolution volume不显示问题
            display_1+=1;
          }
...
/*        if (active) {
  ...
  function setSlice(panel, slice) {
    ...
    //console.log({slice_image:panel.slice_image})
  }
  ...
  function drawCursor(panel, color) {
    ...
    if (panel.anchor) {
      ...
      let value=viewer.volumes[0].getIntensityValue(parseInt(vc.i, 10),parseInt(vc.j, 10),0).toString().slice(0, 3);
      if (value==-2){
        count_1=0;
        count_2=0;
        count_3=0;
        ...
        let tabs=document.getElementById("surface-controls").getElementsByTagName('a');
        tabs[1].click();
      ...
      let infopath = {
          result: 'infoData/info_result2/voxel/' + vc.j.toString().padStart(3,'0')+'_'+vc.i.toString().padStart(3,'0') + '.json',
          //distance: 'infoData/info_result2/voxel/' + vc.j.toString().padStart(3,'0')+'_'+vc.i.toString().padStart(3,'0') + '.json',
          roiName: 'j:' + vc.j.toString().padStart(3,'0') + '; k:' + vc.i.toString().padStart(3,'0')
        };
        $("#metainfo-box").hide();
      let voxelSizeSelect = $("[name=voxelSizeSelect]:checked").val()||'1';
      let CCPath = 'models/Group_result1/' + voxelSizeSelect + '/data_' + vc.j.toString().padStart(3,'0')+'_'+vc.i.toString().padStart(3,'0')  + '.nii';
      console.log(CCPath)
      document.getElementById("volume-controls").getElementsByTagName('a')[0].click();
      //context.restore();  
      let res={index:vc.j+'_'+vc.i,dataPath:CCPath,infopath:infopath};
      viewer.uploadData(res.dataPath,false);
      window.infoviewer.loadInfo(res.infopath);
      //TODO: condition-2 load intensityData
      //if new, upload resultPanel
      if (window.surfviewer.model_data.get().intensity_data[0].name.substr(0, 5) === 'S1200') {
          window.surfviewer.setAttribute("fix_color_range", false);
        }
        window.surfviewer.getDataURL(res.index, window.surfviewer, function(url,viewer,colormap,template){
        window.surfviewer.loadfromURL(url,viewer,colormap,template)
      });
      }
    }
    else if(panel.cpoint){
      ...
      if (panel.canvas.width - cursor.x < 250) {
      ...
      let value=viewer.volumes[1].getIntensityValue(parseInt(vc.i, 10),parseInt(vc.j, 10),0).toString().slice(0, 3);
      if (value>0||value==0){
        context.fillStyle = "rgba(0,0,0,0.8)";
        context.rect(x_rect,y_rect,190,15)
        context.fill()
        voxelinfo=' j: '+vc.i+', k: '+vc.j+', value: '+value;
        context.fillStyle = "#FFFFFF";
        context.fillText(voxelinfo, x, y);

        setTimeout(function(){delete panel.cpoint;context.restore();},1000);
      }
    }
  ...
  function drawSlice(panel) {
    ...
    if (image) {
      ...
      //console.log({origin:origin,image_center_x:panel.image_center.x,image_center_y:panel.image_center.y,slice_image_w:panel.slice_image.width,slice_image_h:panel.slice_image.height})
