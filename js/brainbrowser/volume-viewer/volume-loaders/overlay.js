...
(function() {
  ...
  VolumeViewer.volume_loaders.overlay = function(options, callback) {
    ...
    for (var i = 0; i < order.length; i++) {
      header[order[i]] = {};
      header[order[i]].step = options.header.step[i];
      //header[order[i]].start = -512/;
      header[order[i]].start = options.header.start[i];
      header[order[i]].direction_cosines = [-0, -0, -0];
      header[order[i]].direction_cosines[i] = 1;
      header[order[i]].space_length = options.header.space_length[i];
      header[order[i]].offset = options.header.offset[i]
    }
     ...
     */
    header.order = ["zspace", "yspace", "xspace"]
     ...
     */
    //overlay_volume.size = [311,260];
    overlay_volume.volumes = [];
    overlay_volume.blend_ratios = [1, 1];
    ...
    overlay_volume.slice = function(axis, slice_num, time) {
      console.log("slice function run")
    ...
    overlay_volume.getSliceImage = function(slice, zoom, contrast, brightness) {
      //console.log({slices:slice.slices})
      zoom = zoom || 1;
      var slices = slice.slices;
      var images = [];
      var max_width,max_height;
      //max_width= Math.round(this.size[0] * zoom);
      //max_height= Math.round(this.size[1] * zoom);
      ...
      slices.forEach(function(slice, i) {
        ...
        //max_width = Math.round(this.size[0] * zoom);
        //max_height = Math.round(this.size[1] * ystep * zoom);
        max_width = Math.round(slice.width * xstep * zoom);
        max_height = Math.round(slice.height * ystep * zoom);
        //max_height=target_height;
        //max_width=target_width;
        var error_message;
        //console.log({width:slice.width * xstep,height:slice.height * ystep})
        if (!color_map) {
        ...
        //console.log({min_col:min_col,max_col:max_col,min_row:min_row,max_row:max_row})
        //console.log({max_width:max_width,max_height:max_height,zoom:zoom})
        ...
        //console.log({i_offset:i_offset,j_offset:j_offset,k_offset:k_offset})
        ...
        //console.log({i_size:i_size,j_size:j_size,k_size:k_size})
        ...
        //console.log({w_origin0:w_origin})
        ...
        //console.log({w_origin1:w_origin})
        ...
        color_map.mapVColors(slice_data, {
          min: intensity_min,
          max: intensity_max,
          contrast: contrast,
      ...
      for (x = 0; x < width; x += 1) {
        ...
        for (i = 0; i < num_images; i += 1) {
          ...
          if(y < image.height &&  x < image.width) {
            ...
            target_data[pixel + 3] = target_data[pixel + 3] +
                                      image_data[current + 3]* alphas[i];
            alpha += alphas[i];
