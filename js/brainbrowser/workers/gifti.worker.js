...
(function() {
  ...
  function parse(data) {
    ...
    if (n_tri > 0 && n_vtx > 0) {
       ...
       */
      result.structure = gii.getAnatomicalStructure();
      result.type = 'polygon';
      result.vertices = gii.getPointsDataArray().getData();
      result.split = split;
      if (result.split) {
        result.shapes = [
          {indices: gii.getTrianglesDataArray().getData().slice(0, parseInt(n_tri*3 / 2))},
          {indices: gii.getTrianglesDataArray().getData().slice(parseInt(n_tri*3 / 2), n_tri*3)}
        ];
      }
      else {
