  ...
  BrainBrowser.createColorMap = function(data, options) {
    ...
    */
    var color_map = {
      ...
      * ```
      */
      mapVColors: function(intensity_values, options) {
        options = options || {};
        var min = options.min === undefined ? 0 : options.min;
        var max = options.max === undefined ? 255 : options.max;
        var default_colors = options.default_colors || [0, 0, 0, 0];
        ...
        for (i = 0, count = intensity_values.length; i < count; i++) {
          ...
          if(color_map_index < 0 && color_map_index!=-5) {
            idc = ic * default_color_offset;

            // contrast includes scaling factor
            destination[ic]     = (contrast * 1.0 + brightness).toFixed(10);
            destination[ic + 1] = (contrast * 1.0 + brightness).toFixed(10);
            destination[ic + 2] = (contrast * 1.0 + brightness).toFixed(10);
            destination[ic + 3] = scale    * 1;
          } else if(color_map_index===-5){
            destination[ic]     = (contrast * 0.0 + brightness).toFixed(10);
            destination[ic + 1] = (contrast * 0.0 + brightness).toFixed(10);
            destination[ic + 2] = (contrast * 0.0 + brightness).toFixed(10);
            destination[ic + 3] = 0;
          }
          else {
            // contrast includes scaling factor
      ...
      mapColors: function(intensity_values, options) {
        options = options || {};
        var min = options.min === undefined||null||NaN ? 0 : options.min;
        var max = options.max === undefined||null||NaN ? 255 : options.max;
        ...
        for (i = 0, count = intensity_values.length; i < count; i++) {
          ...
          //This inserts the RGBA values (R,G,B,A) independently
          if(color_map_index===-1) {
            destination[ic]     = contrast * 1     + brightness;
            destination[ic + 1] = contrast * 1 + brightness;
            destination[ic + 2] = contrast * 1 + brightness;
            destination[ic + 3] = scale    * 0;
          }else if(color_map_index===-2){
            destination[ic]     = (contrast * 0.7 + brightness).toFixed(10);
            destination[ic + 1] = (contrast * 0.7 + brightness).toFixed(10);
            destination[ic + 2] = (contrast * 0.7 + brightness).toFixed(10);
            destination[ic + 3] = scale    * 1;
          } else if(color_map_index===-3){
            destination[ic]     = (contrast * 0.999 + brightness).toFixed(10);
            destination[ic + 1] = (contrast * 0.999 + brightness).toFixed(10);
            destination[ic + 2] = (contrast * 0.999 + brightness).toFixed(10);
            destination[ic + 3] = scale    * 1;
          }else {
            // contrast includes scaling factor
            destination[ic]     = contrast * color_map_colors[color_map_index]     + brightness;
      ...
      */
      createElement: function(min, max) {
        ...
        canvas  = createCanvas(colors, 10, 30, flip);
        ...
        // Min mark
        console.log(min)
        context.fillRect(0.5, 10, 1, 10);
        context.fillText(min.toPrecision(3), 0.5, 30);

        // Quarter mark
        context.fillRect(canvas.width / 4, 10, 1, 10);
        context.fillText((min + 0.25 * range).toPrecision(3), 0.25 * canvas.width-10, 30);

        // Middle mark
        context.fillRect(canvas.width / 2, 10, 1, 10);
        context.fillText((min + 0.5 * range).toPrecision(3), 0.5 * canvas.width-10, 30);

        // Three-quarter mark
        context.fillRect(3 * canvas.width / 4, 10, 1, 10);
        context.fillText((min + 0.75 * range).toPrecision(3), 0.75 * canvas.width-10, 30);


        // Max mark
        context.fillRect(canvas.width - 0.5, 10, 1, 10);
        context.fillText(max.toPrecision(3), canvas.width - 25, 30);
    ...
    //   }
    // }
    function getColorMapIndex(value, min, max, increment, clamp, flip, color_map_length, options) {
      var color_map_index;

      if ((value < min || value > max) && !clamp) {
        return -1;
      }else if(value > min && value < max){
        color_map_index = Math.floor(Math.max(0, Math.min((value - min) * (color_map_length / (max-min)), color_map_length - 1)));
        color_map_index *= 4;
        return color_map_index;
      } else {
        color_map_index = Math.floor(Math.max(0, Math.min((value - min) * increment, color_map_length - 1)));
