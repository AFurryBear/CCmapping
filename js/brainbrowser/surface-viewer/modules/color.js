...
// 无效2tong2的次数
var count_2=0;
// intensity_data.length>2的次数，当count_3>=的2时候，遇到5tong5时即显示。
var count_3=0;
// surface显示状态
var display=0;
// volume显示状态
...
BrainBrowser.SurfaceViewer.modules.color = function(viewer) {
  ...
  */
  ...
  viewer.updateColors = function(options) {
    ...
    if (model_data.intensity_data.length > 1) {
      ...
      blend = true;
      if(count_1<3&&count_2==0){
        count_1+=1;
      }else {
        count_2+=1;
      }
      console.log('blend is true')
    } else {
    ...
    function applyColorArray(color_array) {
      ...
      if (blend) {
        ...
        // tong>surface显示(不完美)方法
        // var date=new Date();
        // var m=date.getMilliseconds();    //获取当前毫秒数(0-999)
        // var s=date.getSeconds();    //获取当前毫秒数(0-999)
        // console.log(s+".2tong2."+m);
        if(count_2>=3){
          // console.log("tong无效4："+count_2);
          count_2=0;
          count_1=4;
        }
        else if(count_1==3){
          ...
          // console.log("tong清空："+count_1);
          count_1=0;
        }
        if (count_3>=2){
          // console.log("tong强行显示");
          display=2;
          count_3=0;
        }
      }
  ...
  */
  ...
  function blendColors(intensity_data, model_name) {
    ...
    var alpha;
    let white_C=[1,1,1]
    let grey_B=[0.699999988079071,0.699999988079071,0.699999988079071]
    function equar(a, b) {
    ...
    blended_color = color_arrays[0];

    for (i = 0, num_colors = color_arrays[0].length / 4; i < num_colors; i++){
      for (j = 1, num_arrays = color_arrays.length; j < num_arrays; j++) {
        alpha=alphas[j];
        ci = i * 4;
        if (equar(grey_B,color_arrays[j].slice(ci,ci+3))){
          if(alpha!=0){
            blended_color[ci]     = color_arrays[j][ci] * 1;
            blended_color[ci + 1] = color_arrays[j][ci + 1] * 1;
            blended_color[ci + 2] = color_arrays[j][ci + 2] * 1;
            blended_color[ci + 3] = 1;
          }
        }else if(!equar(white_C,color_arrays[j].slice(ci,ci+3))&&!equar(grey_B,blended_color.slice(ci,ci+3))) {
          blended_color[ci]     = color_arrays[j][ci] * 1;
          blended_color[ci + 1] = color_arrays[j][ci + 1] * 1;
          blended_color[ci + 2] = color_arrays[j][ci + 2] * 1;
          blended_color[ci + 3] = 1;
